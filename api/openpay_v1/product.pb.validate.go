...
func (m *CreateProductRequest) validate(all bool) error {
	...
	if !_CreateProductRequest_Code_Pattern.MatchString(m.GetCode()) {
	...
	if _, ok := _CreateProductRequest_DefaultCurrency_InLookup[m.GetDefaultCurrency()]; !ok {
		err := CreateProductRequestValidationError{
...
var _CreateProductRequest_Code_Pattern = regexp.MustCompile("^[a-z][a-z0-9_]{1,30}$")

var _CreateProductRequest_DefaultCurrency_InLookup = map[string]struct{}{
	"INR": {},
}
...
func (m *UpdateProductRequest) validate(all bool) error {
	...
	if _, ok := _UpdateProductRequest_Status_NotInLookup[m.GetStatus()]; ok {
		err := UpdateProductRequestValidationError{
			field:  "Status",
			reason: "value must not be in list [PRODUCT_STATUS_UNSPECIFIED]",
		}
		if !all {
			return err
		}
	...
	if _, ok := ProductStatus_name[int32(m.GetStatus())]; !ok {
		err := UpdateProductRequestValidationError{
			field:  "Status",
